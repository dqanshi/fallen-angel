...
- These are the minimum required vars need to setup to make RYTHMX functional.

1. `API_ID` : Get it from my.telegram.org 
2. `API_HASH`  : Get it from my.telegram.org 
3. `BOT_TOKEN` : Get it from [@Botfather](http://t.me/BotFather) in Telegram
4. `MONGO_DB_URI` : Get mongo db [from here.](cloud.mongodb.com)
...
3. `VIDEO_STREAM_LIMIT` : Maximum number of video calls allowed on bot. You can later set it via /set_video_limit on telegram. Default to 10 chats.
4. `SERVER_PLAYLIST_LIMIT` : Maximum Limit Allowed for users to save playlists on bot's server. Default to 50
5. `PLAYLIST_FETCH_LIMIT` :  Maximum limit for fetching playlist's track from youtube, spotify, apple links. Default to 50
6. `CLEANMODE_MINS` : Cleanmode time after which bot will delete its old messages from chats. Default to 7 Mins.
...
3. `TELEGRAM_EDIT_SLEEP` : Time sleep duration For Telegram Downloader. Default to 3 seconds
...
- You can play tracks or playlists from spotify from RYTHMX 
...
- If you plan to use RYTHMX with your own customized or modified code.
...
- You can change images which are used in RYTHMX 
- You can use telegaph links here.
...
1. `STRING_SESSION2` : Pyrogram Session Needed.2. `STRING_SESSION3` : Pyrogram Session Needed.3. `STRING_SESSION4` : Pyrogram Session Needed.4. `STRING_SESSION5` : Pyrogram Session Needed.