...
async def varsFunc(client, message):
    mystic = await message.reply_text(
        "ɢᴇᴛᴛɪɴɢ ᴀʟʟ ᴛʜᴇ ᴄᴏɴғɪɢ ᴠᴀʀɪᴀʙʟᴇs..."
    )
    ...
    text = f"""**ʀʏᴛʜᴍx ᴄᴏɴғɪɢ ᴠᴀʀɪᴀʙʟᴇs ʟɪsᴛ:**

**<u>ʙᴀsɪᴄ ᴠᴀʀɪᴀʙʟᴇs:</u>**
`MUSIC_BOT_NAME` : **{bot_name}**
`DURATION_LIMIT` : **{play_duration} ᴍɪɴ**
`SONG_DOWNLOAD_DURATION_LIMIT` :** {song} ᴍɪɴ**
`OWNER_ID` : **{owner_id}**
...
`AUTO_SUGGESTION_TIME` : **{auto_sug} sᴇᴄᴏɴᴅs**
`AUTO_DOWNLOADS_CLEAR` : **{down}**
`PRIVATE_BOT_MODE` : **{pvt}**
`YOUTUBE_EDIT_SLEEP` : **{yt_sleep} sᴇᴄᴏɴᴅs**
`TELEGRAM_EDIT_SLEEP` :** {tg_sleep} sᴇᴄᴏɴᴅs**
`CLEANMODE_MINS` : **{cm} mins**
`VIDEO_STREAM_LIMIT` : **{v_limit} ᴄʜᴀᴛs**
