...
<u>ᴜsᴀɢᴇ:</u>
ᴜsᴇᴅ: `{AppHours}`**ʜ**  `{AppMinutes}`**ᴍ**  [`{AppPercentage}`**%**]

<u>Remaining Quota:</u>
ʟᴇғᴛ: `{hours}`**ʜ**  `{minutes}`**ᴍ**  [`{percentage}`**%**]"""
...
async def update_(client, message, _):
    ...
    if verification == "":
        return await response.edit("ᴅɪᴅ ʏᴏᴜ ʀᴇᴍᴇᴍʙᴇʀ ᴛʜᴀᴛ ʏᴏᴜ ᴅɪᴅ ᴀɴʏ ᴄʜᴀɴɢᴇs ɪɴ ᴛʜᴇ ʀᴇᴩᴏsɪᴛᴏʀʏ ? ɪғ ʏᴇs ɪ ᴄᴀɴ'ᴛ sᴇᴇ ᴛʜᴀᴛ ᴅᴏ ɪᴛ ᴀɢᴀɪɴ ᴀɴᴅ ᴩʀᴇss ᴛʜᴀᴛ ғᴜ*ᴋɪɴɢ ᴄᴏᴍᴍɪᴛ ᴄʜᴀɴɢᴇs ʙᴜᴛᴛᴏɴ ᴇʟsᴇ ғᴜ*ᴋ ᴏғғ.")
    updates = ""
    ...
    _update_response_ = "<b>ɴᴇᴡ ᴜᴩᴅᴀᴛᴇ ɪs ᴀᴠᴀɪʟᴀʙʟᴇ ғᴏʀ ᴛʜᴇ ʙᴏᴛ !</b>\n\n➣ ᴛʀʏɪɴɢ ᴛᴏ ᴩᴜsʜ ᴜᴩᴅᴀᴛᴇs</code>\n\n**<u>Updates:</u>**\n\n"
    _final_updates_ = _update_response_ + updates
    if len(_final_updates_) > 4096:
        url = await Yukkibin(updates)
        nrs = await response.edit(
            f"<b>ɴᴇᴡ ᴜᴩᴅᴀᴛᴇ ɪs ᴀᴠᴀɪʟᴀʙʟᴇ ғᴏʀ ᴛʜᴇ ʙᴏᴛ !</b>\n\n➣ ᴛʀʏɪɴɢ ᴛᴏ ᴩᴜsʜ ᴜᴩᴅᴀᴛᴇs</code>\n\n**<u>ᴜᴩᴅᴀᴛᴇs:</u>**\n\n[ᴄʜᴇᴄᴋᴏᴜᴛ ᴜᴩᴅᴀᴛᴇs]({url})"
        )
    else:
        nrs = await response.edit(
    ...
    if await is_heroku():
        try:
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"{config.MUSIC_BOT_NAME} ʜᴀs ᴊᴜsᴛ ʀᴇsᴛᴀʀᴛᴇᴅ ɪᴛsᴇʟғ. sᴏʀʀʏ ғᴏʀ ᴛʜᴇ ɪssᴜᴇs.\n\nʙᴛᴡ ʏᴏᴜ ᴄᴀɴ sᴛᴀʀᴛ ᴩʟᴀʏɪɴɢ ᴀɢᴀɪɴ ᴀғᴛᴇʀ 15 sᴇᴄs.",
                    )
                    ...
                    pass
            await response.edit(
                f"{nrs.text}\n\nʙᴏᴛ sᴜᴄᴄᴇssғᴜʟʟʏ ᴜᴩᴅᴀᴛᴇᴅ ! ɴᴏᴡ ʟᴇᴛ ᴍᴇ ʀᴇsᴛᴀʀᴛ ᴛᴏ ᴅᴏ ᴛʜᴇ ᴄʜᴀɴɢᴇs ɪɴ ᴛʜᴇ ʙᴏᴛ !"
            )
            os.system(
            ...
            await response.edit(
                f"{nrs.text}\n\nsᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ ᴡʜᴇɴ ᴛʀɪᴇᴅ ᴛᴏ ʀᴇʙᴏᴏᴛ ! ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ ᴏʀ ᴄʜᴇᴄᴋ ᴛʜᴇ ғᴜ*ᴋɪɴɢ ʟᴏɢs."
            )
            return await app.send_message(
                config.LOG_GROUP_ID,
                f"ᴀɴ ᴇxᴄᴇᴩᴛɪᴏɴ ᴏᴄᴄᴜʀᴇᴅ ɪɴ ᴜᴩᴅᴀᴛᴇʀ ᴅᴜᴇ ᴛᴏ : <code>{err}</code>",
            )
    else:
        served_chats = await get_active_chats()
        for x in served_chats:
...
async def restart_(_, message):
    ...
    try:
        ...
        pass
    await response.edit(
        "ʀᴇʙᴏᴏᴛ ᴩʀᴏᴄᴇss sᴛᴀʀᴛᴇᴅ ! ɴᴏᴡ ᴡᴀɪᴛ ғᴏʀ ᴀ ᴍɪɴ ᴀɴᴅ ʟᴇᴛ ᴛʜᴇ ʙᴏᴛ ʀᴇsᴛᴀʀᴛ."
    )
    os.system(f"kill -9 {os.getpid()} && bash start")
